...
template<int Dim>
bool SLPlanner<Dim>::search( Solution<Dim> &sol )
{
    ...
    while ( !state_open_list.empty() )
    {
        ...
        // std::cout << ANSI_COLOR_GREEN "SLPlanner GetNeighbor Time: "  << one_clock.Elapsed_us() << " us"  ANSI_COLOR_RESET << std::endl;
...
bool SLPlanner<Dim>::traceSolution( StateNodePtr<Dim> curr_statenodeptr, Solution<Dim> &sol )
{
    ...
    sol.timecost = curr_statenodeptr->state.time_t;
    sol.timestampcost = curr_statenodeptr->state.timestamp;
    sol.controlcost = sol.totalcost - cp.Wt * sol.timecost;
    ...
    sol.states.push_back( best_child.back()->state );
    return true;
}
                                      ...
                                      std::vector<int> &succ_action_id, std::vector<double> &succ_action_cost, vec_E<Primitive<Dim>> &succ_action_pr )
{
    ...
    for ( unsigned int i = 0; i < cp.all_u_num; i++ )
    {
        ...
        Primitive<Dim> pr = Primitive<Dim>( curr_statenodeptr->state, cp.All_U[i], cp.dt );

        CopterState<Dim> tn = pr.evaluate( cp.dt );
        ...
        double cost = compute_primitive_cost(pr);
        if ( cp.yaw_use )
        {
...
double SLPlanner<Dim>::compute_heuristic( CopterState<Dim> &curr_state )
{
    ...
    {
        // double dis = (gmap.toGrid(curr_state.pos) - gmap.toGrid(goal.pos)).template lpNorm<Eigen::Infinity>() * gmap.getRes();
...
bool SLPlanner<Dim>::PrimitiveObstacleChecker( Primitive<Dim> &pr, Vecf<Dim> &obs )
{
    ...
    if ( RootFinder::countRoots( coeffs, 0, pr.getDT() ) > 0 )
    {
        return true;
    }
...
double SLPlanner<Dim>::compute_penalty_factor_by_vel( CopterState<Dim> &curr_state, CopterState<Dim> &next_state )
{
    ...
    if ( v1.norm() > 1e-5 && v2.norm() > 1e-5 )
    {
        ...
        // double angle_v1 = std::atan2( v1(1), v1(0) );
        // double angle_v2 = std::atan2( v2(1), v2(0) );
        // double delta_angle = std::abs( angle_v1 - angle_v2 );
        // double k = 1;
        // if ( delta_angle <= M_PI / 6  )
        //     res = 1;
        // else if ( delta_angle <= M_PI / 4 )
        //    res = k * power( (delta_angle/(M_PI/6)), 2 )  ;
        //     // res = 1.5;
        // else
        //     res = 3;
...
double SLPlanner<Dim>::compute_penalty_factor_by_yaw(  CopterState<Dim> &next_state )
{
    ...
    if( v.norm() > 1e-5 & cp.yaw_use )
    {
        ...
        // double angle_v = std::atan2( v(1), v(0) );
        // double delta_angle = std::abs( angle_v - next_state.yaw );
        // double k = 1;
        // if ( delta_angle <= (cp.max_yaw / 3)  )
        //     res = 1;
        // else if ( delta_angle <= (cp.max_yaw / 2) )
        //    res = k * power( (delta_angle/(cp.max_yaw / 3)), 2 )  ;
        //     // res = 1.5;
        // else
        //     res = 4;

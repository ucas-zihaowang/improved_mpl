...
template<int Dim>
GlobalMap<Dim>::GlobalMap(const Veci<Dim> &dim, double res, const Vecf<Dim> &origin, CopterGeo<Dim> &UAVGeoDes_, ObsGeo<Dim> &ObsGeoDes_):UAVGeoDes(UAVGeoDes_),ObsGeoDes(ObsGeoDes_)
{
    this->dim = dim;
    this->res = res;
    this->origin = origin;
